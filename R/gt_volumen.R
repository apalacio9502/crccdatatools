...
#' Gráfica el volumen operado (barras h)
#'
...
gt_vol<- function(datos,colores,fixedrange=FALSE,boton_activo=NULL,botones_inactivos=c()){
  ...
  if (nrow(datos)>0) {
    ...
    # Se grafica el volumen operado (Fecha Especifica)
...
#' Gráfica el volumen operado por miembro (barras)
#'
...
gt_vol_por_miembro<- function(datos,colores,fixedrange=FALSE,boton_activo=NULL,botones_inactivos=c()){
  ...
  if (nrow(datos)>0) {
    ...
    # Se grafica el volumen operado por miembro (Fecha Especifica)
    plot <- plot_ly(data= datos_completos ,x=~MIEMBRO_ID_SEUDONIMO,colors = lista_colores,color=~COLOR_ID,
                    transforms = list(list(type = 'filter',target = 'y',operation = ')(',value = 0)),
                    textposition = 'none',source="MOD_GE_SUB_IO_C1_D_plot_2") %>%
...
#' Gráfica el volumen operado diario (lineas)
#'
...
gt_vol_diario<- function(datos,colores,fixedrange=FALSE,dash_board=TRUE,boton_activo=NULL,botones_inactivos=c()){
  ...
  if (nrow(datos)>0) {
    ...
    # Se grafica el volumen operado diario
...
#' Gráfica el volumen operado promedio diario por (Mes o Año) (barras)
#'
...
gt_vol_promedio_diario<- function(datos,colores,fixedrange=FALSE,dash_board=TRUE,promedio="m",boton_activo=NULL,botones_inactivos=c()){
  ...
  if (nrow(datos)>0) {
    ...
    # Se grafica el volumen operado promedio diario por (Mes o Año)
...
#' Gráfica el volumen operado promedio diario por (Mes o Año) y tipo de cuenta (barras)
#'
  ...
  if (nrow(datos)>0) {
    ...
    datos_completos <- datos %>% rename(TIPO="CUENTA_GARANTIA_TIPO",ID="PRODUCTO_SUBTIPO")  %>%
      ...
      mutate(TEXTO_1=paste(VALOR_1,"Billones /",dt_porcentaje_caracter(VALOR_1/sum(VALOR_1)), "P /",CAMBIO_VALOR_1,"C"),
             TEXTO_2=paste(VALOR_2,"Billones /",dt_porcentaje_caracter(VALOR_2/sum(VALOR_2)),"P /",CAMBIO_VALOR_2,"C")) %>% ungroup() %>%
      left_join(tipos %>% select(TIPO,POSICION,VISIBLE_1,VISIBLE_2),by="TIPO") %>%
      mutate(ORDENADOR=as.numeric(fct_reorder(factor(paste0(POSICION,"-",ID)),VALOR_1-VALOR_2,.fun=mean,.desc=T)),
             COLOR_ID=paste0(POSICION,"-",if_else(nchar(ORDENADOR)==1,"0",""),ORDENADOR)) %>% arrange(COLOR_ID)
    ...
    # Se grafica el volumen operado promedio diario por (Mes o Año) y tipo de cuenta
    plot <- plot_ly(data= datos_completos ,x=~FECHA_FORMATO,colors = lista_colores,color=~COLOR_ID,
                    transforms = list(list(type = 'filter',target = 'y',operation = ')(',value = 0)),
                    textposition = 'none') %>%
      add_bars(y=~VALOR_1,text=~TEXTO_1,visible=~VISIBLE_1,
...
#' Gráfica el ranking de los miembros por volumen operado (barras)
#'
...
gt_vol_ranking_miembros <- function(datos,colores,fixedrange=FALSE,boton_activo=NULL){
  ...
  if (nrow(datos)>0) {
    ...
    datos_completos <- datos %>%
      ...
      mutate(ORDENADOR=factor(paste(TIPO,"-",ID)),
             MIEMBRO_ID_SEUDONIMO=fct_reorder(factor(MIEMBRO_ID_SEUDONIMO),if_else(TIPO=="General",VALOR,0),.fun=sum,.desc=T),
             COLOR_ID=paste0(POSICION,"-",if_else(nchar(as.numeric(ORDENADOR))==1,"0",""),as.numeric(ORDENADOR))) %>% arrange(COLOR_ID)
    ...
    # Se grafica la volumen operado por miembro y por producto origen
    plot <- plot_ly(data= datos_completos ,x=~MIEMBRO_ID_SEUDONIMO,colors = lista_colores,color=~COLOR_ID,
                    transforms = list(list(type = 'filter',target = 'y',operation = '>',value = 0)),
...
#' Gráfica el número de miembros operando en cada segmento por mes (barras)
#'
...
gt_miembros_operando<- function(datos,colores,fixedrange=FALSE){
  ...
  if (nrow(datos)>0) {
    ...
    datos_completos <- datos %>%
      ...
      group_by(FECHA_ANO_MES,TIPO="SEGMENTO_NOMBRE",ID=SEGMENTO_NOMBRE) %>%
      summarise(NUMERO_MIEMBROS=n_distinct(MIEMBRO_ID_SEUDONIMO),.groups = "drop")

    # Se crea la lista de colores
    lista_colores <- datos_completos %>% distinct(TIPO,ID) %>%
      left_join(colores,by = c("TIPO", "ID")) %>% arrange(ID) %>% pull(COLOR)

    # Se grafica los el numero de miembros operando por segmento y mes
    plot <- plot_ly(data= datos_completos ,x=~FECHA_ANO_MES,colors = lista_colores,color=~ID) %>%
