...
dt_gen_pa_resumen <- function(conexion,periodo_analisis=NULL,fecha_analisis=NULL,ficticio=FALSE,segmentos=NULL,oracle=FALSE){
  ...
  datos <-  dbGetQuery(conexion , glue("SELECT FECHA, SEGMENTO_ID,
                                    ...
                                    PRODUCTO_ORIGEN,POSICION_COMPRADORA_VALORADA,
                                    POSICION_VENDEDORA_VALORADA FROM MOD_GE_SUB_IO_POSICION_ABIERTA_RESUMEN
  ...
  datos <- datos %>% mutate(FECHA=ymd(FECHA)) %>%
    complete(FECHA,nesting(SEGMENTO_ID, SEGMENTO_NOMBRE,MIEMBRO_ID_SEUDONIMO,MIEMBRO_TIPO, CUENTA_GARANTIA_TIPO,
             ...
             fill = list(POSICION_COMPRADORA_VALORADA=0,POSICION_VENDEDORA_VALORADA=0)) %>%
